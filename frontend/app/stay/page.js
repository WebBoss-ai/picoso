...
import { ArrowUpRight, Leaf, MapPin, Sparkles } from 'lucide-react';
import { DESTINATIONS } from '@/lib/stayStore';
        ...
        <section className="pt-10 sm:pt-16 pb-12 sm:pb-16">
          <div className="relative overflow-hidden rounded-[2rem] border border-gray-200 bg-white min-h-[420px] sm:min-h-[520px]">
            ...
            <div className="relative h-full min-h-[420px] sm:min-h-[520px] flex flex-col justify-end p-6 sm:p-10 md:p-14">
              ...
              <h1 className="text-4xl sm:text-5xl md:text-6xl font-semibold text-white tracking-tight max-w-xl leading-[1.05]">
                Picoso Stay
              </h1>
              ...
              <div className="mt-7 flex flex-wrap gap-3">
                <a
                  href="#destinations"
                  className="h-11 px-5 rounded-2xl bg-turtle-500 hover:bg-turtle-400 text-white text-sm font-semibold inline-flex items-center gap-2 transition-colors"
                >
          ...
          {[
            {
              ...
              body: 'No advance payment. Visit, experience, then settle at the stay.',
              icon: Sparkles,
            },
            {
              title: 'Check-in OTP',
              body: 'A 4-digit code at booking — share it on arrival for seamless check-in.',
              icon: Leaf,
            },
            {
              title: 'Curated for nature',
              body: 'Cabins, cottages, glamping & homestays chosen for quiet & forests.',
              icon: MapPin,
            },
        ...
        <section id="destinations" className="pb-20">
          <div className="flex items-end justify-between gap-4 mb-6">
            <div>
              ...
              <h2 className="text-2xl sm:text-3xl font-semibold text-gray-900 tracking-tight mt-1">
                Where will you stay?
              </h2>
            </div>
          </div>
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {DESTINATIONS.map((d) => {
              const inner = (
                <div
                  className={`group relative overflow-hidden rounded-[1.5rem] border border-gray-200 bg-white aspect-[4/5] sm:aspect-[3/4] ${
                    d.available ? 'cursor-pointer' : 'opacity-90'
                  }`}
                >
                  <div
                  ...
                  <div className="absolute top-4 left-4 right-4 flex justify-between">
                    {d.available ? (
                      <span className="px-2.5 py-1 rounded-lg bg-turtle-500 text-white text-[10px] font-semibold tracking-wide">
                        Open
                      </span>
                    ) : (
                      <span className="px-2.5 py-1 rounded-lg bg-white/15 border border-white/25 text-white text-[10px] font-semibold tracking-wide backdrop-blur-sm">
                        Coming soon
                      </span>
                    )}
                    {d.available && (
                      <span className="px-2.5 py-1 rounded-lg bg-white/15 border border-white/25 text-white text-[10px] font-medium backdrop-blur-sm">
                        {d.stays} stays
                      </span>
                    )}
                  </div>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-6">
                    ...
                    {d.available && (
                      <span className="mt-4 inline-flex items-center gap-1.5 text-xs font-semibold text-turtle-300 group-hover:text-turtle-200">
                        View stays <ArrowUpRight size={14} />
                      </span>
                    )}
                  </div>
                </div>
